...
class Dao {
    ...
    public static function getPDO() {
        try {
            $pdo = new \PDO('mysql:host=localhost;dbname=stock-management', "root", "");
            return $pdo;
        } catch (\Throwable) {
...
GROUP BY num_com;";
    ...
    public static function topSales() {
        $pdo = Dao::getPDO();
        $sql = "SELECT SUM(`qte_pr` * prix_vente) as total, num_pr, num_com, nom, prenom, image, date_com, nom,prenom,num_com FROM (((contient_pr NATURAL JOIN produit) NATURAL JOIN commande) NATURAL JOIN client) WHERE id_cli = id GROUP BY num_com ORDER BY total DESC;";
        return $pdo->query($sql)->fetchAll();
    }
}